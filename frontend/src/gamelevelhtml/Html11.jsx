import React from "react";
import background from "../picture/background.png";
import Gamebutton from "./Gamebutton";
import LayoutGame from "./LayoutGame";
import TitleHTML from "./TitleHTML";

const Html11 = () => {
  return (
    <div
      className="w-full h-screen relative flex items-center justify-center flex-col"
      style={{
        backgroundImage: `url(${background})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundRepeat: "no-repeat",
      }}
    >
      <LayoutGame>
        <div>
          <TitleHTML title={"LEVEL 11"} />
        </div>

        <div className="flex items-center justify-center flex-col relative space-y-2 font-mono">
          <div className="font-bold text-4xl mt-2">
            <h1>Level 11: Links with Attributes</h1>
          </div>
          <div className="p-5 md:py-1 px-5 text-lg text-center md:text-1xl leading-relaxed text-[#414040] ">
            <p>
              The &lt;a&gt; tag can have additional attributes like target,
              which defines how the link will open (e.g., in a new tab).
            </p>
          </div>

          <div className="p-5 text-1xl  leading-relaxed text-[#ffffff] bg-[#222] rounded-md  ">
            <div className="">
              <p>Example:</p>
              <p>
                &lt;html&gt; <br></br>
                &lt;body&gt;
                <br></br>
                &lt;a href="https://www.nasa.gov" alt="_blank"&gt; Visit NASA
                for more information &lt;a&gt;
                <br></br>
                &lt;/body&gt; <br></br>&lt;/html&gt;
              </p>
            </div>
          </div>
        </div>

        <Gamebutton></Gamebutton>
      </LayoutGame>
    </div>
  );
};

export default Html11;
