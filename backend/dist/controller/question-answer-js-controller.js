...
const js_model_1 = require("../model/js-model");
const createQuestion = async (req, res, next) => {
    try {
        const { category, level, question, answer1, answer2, answer3, answer4 } = req.body;
        const createQuestion = await js_model_1.jsQuestionModel.create({
            category,
            level,
            question,
            answer1,
...
const getByCategoryAndLevel = async (req, res, next) => {
    try {
        ...
        const question = await js_model_1.jsQuestionModel.findOne({
            category,
            level,
        });
        if (!question) {
