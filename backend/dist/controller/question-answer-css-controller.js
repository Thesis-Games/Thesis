...
const css_model_1 = require("../model/css-model");
const createQuestion = async (req, res, next) => {
    try {
        const { category, level, question, answer1, answer2, answer3, answer4 } = req.body;
        const createQuestion = await css_model_1.cssQuestionModel.create({
            category,
            level,
            question,
            answer1,
...
const getByCategoryAndLevel = async (req, res, next) => {
    try {
        ...
        const question = await css_model_1.cssQuestionModel.findOne({
            category,
            level,
        });
        if (!question) {
